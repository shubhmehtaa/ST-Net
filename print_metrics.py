...
def test(name):
	...
	GT_img = nib.load(os.path.join('/media/sail/Elements/BME_Grad_Project/Joanne/BBB_DL/demo/data/raw_process/ktrans_th', name[-17:-12] + '_HighDose_ktrans_th.nii.gz')).get_fdata()
	BrainMask = nib.load(os.path.join('/media/sail/Elements/BME_Grad_Project/Joanne/BBB_DL/demo/data/raw/BrainMask', BrainMask_ID[name[-17:-12]] + '.nii.gz')).get_fdata()
...
for f in range(1, len(set(fold_df['fold']))+1):
	...
	for pat_dir in testing_subject:
		...
		pat_result, loss_dict = test(save_name)
